 ...
 * @author h7moreir
 */
public class TransitionToAttack extends UT2004BotModuleController<UT2004Bot> implements Transition {

    @Override
    public HunterState transition(HunterBot Bot) {
        Item item = Bot.getItems().getPathNearestSpawnedItem(ItemType.Category.HEALTH);
        if(Bot.shouldEngage && Bot.getPlayers().canSeeEnemies() && Bot.getWeaponry().hasLoadedWeapon()){
